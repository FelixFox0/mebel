...
class ModelCheckoutOrder extends Model {
    ...
    public function editOrderEmail($order_id, $email)
    {
        // Void the order first
        $this->addOrderHistory($order_id, 0);

        $this->db->query("UPDATE `" . DB_PREFIX . "order` SET email = '" . $this->db->escape($email) . "', date_modified = NOW() WHERE order_id = '" . (int)$order_id . "'");
    }
